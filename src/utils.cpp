  ...
  try {
    ...
    for (int cam = 0; cam < numCameras; cam++) {
      ...
      if (mkdir(ss.str().c_str(), S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH) < 0) {
        cout << "Failed to create directory " << ss.str()
             << "! Data will be written into pre existing directory if it "
                "exists..."
             << endl;
      }
    }
    ...
    for (int imageCnt = 0; imageCnt < k_numImages; imageCnt++) {
      ...
      try {
        ...
        for (unsigned int i = 0; i < numCameras; i++) {
          ...
          if (pResultImages[i]->IsIncomplete()) {
            ...
            cout << "Image incomplete with image status "
                 << pResultImages[i]->GetImageStatus() << "..." << endl
                 << endl;
            return -1;
          }
        ...
        for (unsigned int i = 0; i < numCameras; i++) {
          ...
          LOG(INFO) << "Grabbed image " << imageCnt << ", width = " << width
                    << ", height = " << height << ", timestamp: " << int(result)
                    << "-" << pResultImage->GetTimeStamp() << ", cam = " << i
                    << ", frameID = " << pResultImage->GetFrameID() << endl;
          ...
          // filename<<"cam"<<i<<"/"<< string(num) << "-" << int(result)+int(it)
          // << int(ft*1000000000) << ".png";
          filename << "cam" << i << "/" << int(result) + int(it)
                   << int(ft * 1000000000) << ".jpg";
...
int initializeCameras(CameraList camList, int numCameras,
  ...
  for (int i = numCameras - 1; i >= 0; i--) {
    ...
    try {
      ...
      if (!IsAvailable(ptrAcquisitionMode) || !IsWritable(ptrAcquisitionMode)) {
        ...
        cout << "Unable to set acquisition mode to continuous (enum "
                "retrieval). Aborting..."
             << endl
             << endl;
        return -1;
      ...
      LOG(INFO) << "Acquisition mode set to continuous for camera: " << i << " "
                << endl;
      ...
      //  cout << "Camera " << i << " linkspeed is " << pCam->DeviceLinkSpeed <<
      //  endl;
...
CameraList sort_cam_list(CameraList camList) {
  ...
  for (int camno = 0; camno < sizeof(CAMERA_SNs) / sizeof(CAMERA_SNs[0]);
       camno++) {
