...
void acquisition::Camera::init() { pCam_->Init(); }

void acquisition::Camera::deinit() { pCam_->DeInit(); }
...
ImagePtr acquisition::Camera::grab_frame() {
  ...
  if (pResultImage->IsIncomplete()) {
    ...
    if (frameID_ >= 0) {
      ...
      ROS_WARN_STREAM_COND(frameID_ > lastFrameID_ + 1,
                           "Frames are being skipped!");
    } else {
      frameID_ = pResultImage->GetFrameID();
      ROS_ASSERT_MSG(
          frameID_ == 0,
          "First frame ID was not zero! Might cause sync issues later...");
    }
  }

  ROS_DEBUG_STREAM("Grabbed frame from camera "
                   << get_id() << " with timestamp " << timestamp_ * 1000);
...
int acquisition::Camera::get_frame_id() { return frameID_; }
...
Mat acquisition::Camera::convert_to_mat(ImagePtr pImage) {
  ...
  Mat img;
  if (COLOR_)
    img = Mat(colsize + YPadding, rowsize + XPadding, CV_8UC3,
              convertedImage->GetData(), convertedImage->GetStride());
  else
    img = Mat(colsize + YPadding, rowsize + XPadding, CV_8UC1,
              convertedImage->GetData(), convertedImage->GetStride());
...
void acquisition::Camera::end_acquisition() {
  ...
  if (pCam_->GetNumImagesInUse())
    ROS_WARN_STREAM("Some images still currently in use! Use image->Release() "
                    "before deinitializing.");
...
void acquisition::Camera::setEnumValue(string setting, string value) {
  ...
  if (!IsAvailable(ptr) || !IsWritable(ptr))
    ROS_FATAL_STREAM("Unable to set " << setting << " to " << value
...
void acquisition::Camera::setIntValue(string setting, int val) {
  ...
  if (!IsAvailable(ptr) || !IsWritable(ptr)) {
    ROS_FATAL_STREAM("Unable to set " << setting << " to " << val
...
void acquisition::Camera::setResolutionPixels(int width, int height) {
  ...
  if (!IsAvailable(ptrWidth) || !IsWritable(ptrWidth)) {
    ROS_FATAL_STREAM("Unable to set width"
                     << "). Aborting...");
    return;
  }
  ...
  if (!IsAvailable(ptrHeight) || !IsWritable(ptrHeight)) {
    ROS_FATAL_STREAM("Unable to set height"
                     << "). Aborting...");
    return;
  }
...
void acquisition::Camera::adcBitDepth(gcstring bitDep) {
  ...
  if (!IsAvailable(ptrADC) || !IsWritable(ptrADC)) {
    ROS_FATAL_STREAM("Unable to set ADC Bit "
                     << "). Aborting...");
    return;
  }
...
void acquisition::Camera::setBufferSize(int numBuf) {
  ...
  if (!IsAvailable(StreamNode) || !IsWritable(StreamNode)) {
    ROS_FATAL_STREAM("Unable to set StreamMode "
                     << "). Aborting...");
    return;
  }
...
void acquisition::Camera::setISPEnable() {
  ...
  if (!IsAvailable(ptrISPEn) || !IsWritable(ptrISPEn)) {
    ROS_FATAL_STREAM("Unable to set ISP Enable (node retrieval; camera "
                     << "). Aborting...");
    return;
  }
...
void acquisition::Camera::setFREnable() {
  CBooleanPtr ptrAcquisitionFrameRateEnable =
      pCam_->GetNodeMap().GetNode("AcquisitionFrameRateEnable");
  if (!IsAvailable(ptrAcquisitionFrameRateEnable) ||
      !IsWritable(ptrAcquisitionFrameRateEnable)) {
    ROS_FATAL_STREAM("Unable to set frameRateEnable (node retrieval; camera "
                     << "). Aborting...");
    return;
  }
...
void acquisition::Camera::exposureTest() {
  ...
  if (!IsAvailable(ptrExpTest) || !IsReadable(ptrExpTest)) {
    ROS_FATAL_STREAM("Unable to set exposure "
                     << "). Aborting..." << endl
                     << endl);
    return;
  }
