...
acquisition::Capture::Capture() : nh_(), nh_pvt_("~") {
  ...
  if (usb_mem) {
    ...
    if (mem >= 1000)
      ...
      ROS_FATAL_STREAM("  USB memory on system too low ("
                       << mem << " MB)! Must be at least 1000 MB. Run: \nsudo "
                                 "sh -c \"echo 1000 > "
  ...
  acquisition_pub =
      nh_.advertise<spinnaker_sdk_camera_driver::spinnaker_image_names>(
          "camera", 1000);
}
...
void acquisition::Capture::load_cameras() {
  ...
  for (int j = 0; j < cam_ids_.size(); j++) {
    ...
    for (int i = 0; i < numCameras_; i++) {
      ...
      if (cam.get_id().compare(cam_ids_[j]) == 0) {
        ...
        camera_image_pubs.push_back(nh_.advertise<sensor_msgs::Image>(
            "camera_array/" + cam_names_[j] + "/image_raw", 1));
        camera_info_pubs.push_back(nh_.advertise<sensor_msgs::CameraInfo>(
            "camera_array/" + cam_names_[j] + "/camera_info", 1));
        ...
        if (PUBLISH_CAM_INFO_) {
          ...
          ci_msg.P = {intrinsic_coeff_vec_[j][0], intrinsic_coeff_vec_[j][1],
  ...
  // Have to reset number of cameras based on the setting from the param file
  // not from the list of available cameras
  if (found_all_cams) {
...
void acquisition::Capture::read_parameters() {
  ...
  int mcam_int;
  if (nh_pvt_.getParam("master_cam", mcam_int))
    ROS_INFO("master_cam is required!");
  ...
  if (nh_pvt_.getParam("utstamps", MASTER_TIMESTAMP_FOR_ALL_)) {
    ...
    ROS_INFO("  Unique time stamps for each camera: %d",
             !MASTER_TIMESTAMP_FOR_ALL_);
  } else
    ROS_WARN(
        "  'utstamps' Parameter not set, using default behavior utstamps=%d",
        !MASTER_TIMESTAMP_FOR_ALL_);

  if (nh_pvt_.getParam("color", color_))
    ROS_INFO("  color set to: %d", color_);
  else
    ROS_WARN("  'color' Parameter not set, using default behavior color=%d",
             color_);
  ...
  if (nh_pvt_.getParam("to_ros", EXPORT_TO_ROS_))
    ...
    ROS_WARN("  'to_ros' Parameter not set, using default behavior to_ros=%d",
             EXPORT_TO_ROS_);

  if (nh_pvt_.getParam("live", LIVE_))
    ROS_INFO("  Showing live images setting: %d", LIVE_);
  else
    ROS_WARN("  'live' Parameter not set, using default behavior live=%d",
             LIVE_);
  ...
  if (nh_pvt_.getParam("live_grid", GRID_VIEW_)) {
    ...
    ROS_WARN(
        "  'live_grid' Parameter not set, using default behavior live_grid=%d",
        GRID_VIEW_);
  ...
  if (nh_pvt_.getParam("time", TIME_BENCHMARK_))
    ...
    ROS_WARN("  'time' Parameter not set, using default behavior time=%d",
             TIME_BENCHMARK_);
  ...
  if (nh_pvt_.getParam("skip", skip_num_)) {
    if (skip_num_ > 0)
      ...
      skip_num_ = 20;
      ROS_WARN(
          "  Provided 'skip'=%d is not valid, using default behavior, skip=%d",
          skip_num_);
    }
  } else
  ...
  if (nh_pvt_.getParam("delay", init_delay_)) {
    if (init_delay_ >= 0)
      ROS_INFO("  Init sleep delays set to : %d sec", skip_num_);
    else {
      init_delay_ = 1;
      ROS_WARN("  Provided 'delay'=%d is not valid, using default behavior, "
               "delay=%d",
               init_delay_);
    }
  } else
    ROS_WARN("  'delay' Parameter not set, using default behavior: delay=%d",
             skip_num_);

  if (nh_pvt_.getParam("fps", master_fps_)) {
    if (master_fps_ >= 0)
      ROS_INFO("  Master cam fps set to : %0.2", master_fps_);
    else {
      master_fps_ = 20;
      ROS_WARN(
          "  Provided 'fps'=%0.2f is not valid, using default behavior, fps=%d",
          master_fps_);
    }
  } else
    ROS_WARN("  'fps' Parameter not set, using default behavior: fps=%0.2f",
             master_fps_);
  ...
  if (nh_pvt_.getParam("exp", exposure_time_)) {
    if (exposure_time_ > 0)
      ...
      ROS_INFO("  'exp'=%0.f, Setting autoexposure", exposure_time_);
  } else
    ROS_WARN("  'exp' Parameter not set, using default behavior: Automatic "
             "Exposure ");
  ...
  if (nh_pvt_.getParam("soft_framerate", soft_framerate_)) {
    ...
    ROS_WARN("  'soft_framerate' Parameter not set, using default behavior: No "
  ...
  if (SAVE_ || MAX_RATE_SAVE_) {
    if (nh_pvt_.getParam("frames", nframes_)) {
      ...
      ROS_WARN("    'frames' Parameter not set, using defult behavior, frames "
  ...
  for (int i = numCameras_ - 1; i >= 0; i--) {
    ...
    try {
      ...
      if (!soft) {
        ...
        cams[i].setBoolValue("ReverseX", reverse_x_);
        cams[i].setBoolValue("ReverseY", reverse_y_);
      ...
      if (error_msg.find("Unable to set PixelFormat to BGR8") >= 0)
        ROS_WARN("Most likely cause for this error is if your camera can't "
                 "support color and your are trying to set it to color mode");
      ros::shutdown();
    }
  }
  ...
  for (int i = 0; i < numCameras_; i++) {
    ...
    if (mkdir(ss.str().c_str(), S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH) < 0) {
      ROS_WARN_STREAM("Failed to create directory "
                      << ss.str() << "! Data will be written into pre existing "
...
void acquisition::Capture::export_to_ROS() {
  ...
  for (unsigned int i = 0; i < numCameras_; i++) {
    ...
    if (color_)
      img_msgs[i] =
          cv_bridge::CvImage(img_msg_header, "bgr8", frames_[i]).toImageMsg();
    else
      img_msgs[i] =
    ...
    while (ros::ok()) {
      ...
      if (LIVE_) {
        if (GRID_VIEW_) {
          ...
          char title[50];
          sprintf(title, "cam # = %d, cam ID = %s, cam name = %s", CAM_,
                  cam_ids_[CAM_], cam_names_[CAM_]);
      ...
      ROS_INFO_COND(
          TIME_BENCHMARK_,
          "Times (ms):- grab: %.1f, disp: %.1f, save: %.1f, exp2ROS: %.1f",
          toMat_time_ * 1000, disp_time_ * 1000, save_mat_time_ * 1000,
          export_to_ROS_time_ * 1000);
...
void acquisition::Capture::write_queue_to_disk(queue<ImagePtr> *img_q,
  ...
  while (imageCnt < k_numImages) {
    //     ROS_DEBUG_STREAM("  Write Queue to Disk for cam: "<< cam_no <<" size
    //     = "<<img_q->size());
    ...
    ostringstream filename;
    filename << path_ << cam_names_[cam_no] << "/" << cam_names_[cam_no] << "_"
             << id << "_" << todays_date_ << "_" << std::setfill('0')
    ...
    for (int i = 0; i < numCameras_; i++) {
      try {
        ...
        filename << cam_names_[i] << "_" << cam_ids_[i].c_str() << "_"
                 << todays_date_ << "_" << std::setfill('0') << std::setw(6)
                 << imageCnt << "_" << timeStamp << ext_;
        ...
        ROS_DEBUG_STREAM("Queue no. " << i
                                      << " size: " << img_qs->at(i).size());
